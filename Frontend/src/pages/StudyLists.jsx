...
export default function StudyLists() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-algo-950 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto flex items-center justify-center">
              ...
              <Card key={list._id}>
                <CardContent className="p-0">
                  ...
                  <div className="p-4 border-b border-glass-border">
                    ...
                    <div className="h-1.5 bg-glass rounded-full overflow-hidden mt-1.5">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-purple-bright to-indigo-bright transition-all duration-500"
                    ...
                    {availableProblems.length > 0 && (
                      <div className="relative">
                        <Button
                          ...
                          onClick={() => setAddingToList(addingToList === list._id ? null : list._id)}
                        >
                          <span>Add Problem</span>
                          <span className="text-muted">+</span>
                        </Button>
                        {addingToList === list._id && (
                          <div className="absolute top-full left-0 right-0 z-10 mt-1 max-h-40 overflow-y-auto bg-algo-800 border border-glass-border rounded-lg shadow-xl">
                            {availableProblems.slice(0, 20).map((p) => (
                              <button
                                key={p._id}
                                onClick={() => handleAddProblem(list._id, p._id)}
                                className="w-full text-left px-3 py-2 text-xs text-white hover:bg-glass-hover transition-colors truncate"
                              >
                                {p.title}
                              </button>
                            ))}
                  ...
                  {expanded === list._id && (
                    <div className="border-t border-glass-border divide-y divide-glass-border">
                      {list.problems.map((p) => (
                        <div key={p._id} className="flex items-center justify-between px-4 py-2.5 hover:bg-glass-hover transition-colors group">
                          ...
                          <div className="flex items-center gap-2 shrink-0 ml-2">
                            ...
                            <button
                              onClick={() => handleRemoveProblem(list._id, p._id)}
